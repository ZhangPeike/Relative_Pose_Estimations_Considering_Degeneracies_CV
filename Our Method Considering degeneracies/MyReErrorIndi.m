function [ Error,ErrorSum ] = MyReErrorIndi( matches,reprojection )
%Input 输入：matches，原图像提取的特征点对坐标，n对点，矩阵4*n，reprojection为重投影生成的点对坐标，格式同matches
%Output输出：原始点与对应重投影点的欧氏距离的平方和
%不可作为优化指标
npts=size(matches,2);
for i=1:npts
    Error(i)=(matches(1,i)-reprojection(1,i))^2+(matches(2,i)-reprojection(2,i))^2+(matches(3,i)-reprojection(3,i))^2+(matches(4,i)-reprojection(4,i))^2;   
end
